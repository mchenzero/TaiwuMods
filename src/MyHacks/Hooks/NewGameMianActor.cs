...
{
    public static class NewGameMianActor
    {
        [GeneratorPatch(typeof(NewGame), "MakeNewGameMianActor")]
        public static class MakeNewGameMianActorPatch
        {
            ...
            public static void PostMoveNext(bool __result)
            {
                if (__result == false && Main.Enabled && !AlreadyInPatch)
                {
                    ...
                    {
                        AlreadyInPatch = true;
                        RunPostMakeNewGameMianActorHooks();
                    }
                    finally
                    {
        ...
        private static void RunPostMakeNewGameMianActorHooks()
        {
            var methods = Assembly.GetExecutingAssembly().GetTypes()
                            .Where(t => t.Namespace == "MyHacks.Hacks" && t.IsClass)
                            .Select(t => t.GetMethod("PostMakeNewGameMianActor", BindingFlags.Public | BindingFlags.Static))
                            .Where(m => m != null)
                            .ToList();
